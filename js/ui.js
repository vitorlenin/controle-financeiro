// ui.js - Utilitários de UI/DOM + helpers globais (Compat)
// Mantém o app em HTML/CSS/JS puro, sem build.
...
class Money {
  static toNumber(v) {
    ...
    // Accepts: "1.234,56", "1234.56", "R$ 1.234,56", "- 20,00"
    const s = String(v ?? "")
      .replace(/\s+/g, "")
      .replace(/R\$/i, "")
      .replace(/[^0-9,.-]/g, "")
      // If string uses comma as decimal, remove dots as thousands separators
      .replace(/\.(?=\d{3}(?:\D|$))/g, "")
      .replace(",", ".");
    const n = Number(s);
...
class Dates {
  ...
  static monthKeyFromISO(dateISO) {
    const s = String(dateISO || this.todayISO());
    return s.slice(0, 7);
  }
...
class UI {
  ...
  _bind() {
      ...
      try {
        ...
        if (this.state.editTxId) {
          ...
          if (wantsRecurring && !wasRecurring) {
            ...
            this.toast("Alterações salvas ✅ (marcado como recorrente)");
            this.quickMsg.textContent = "";
          } else {
          ...
          if (isSaida && payMethod === "credito" && n > 1) {
            ...
            this.toast(`Salvo em ${n} parcelas ✅`);
            this.quickMsg.textContent = "";
          } else {
        ...
        this.toast(e.message || "Erro ao salvar.", "error");
    ...
    if (this.qPayMethod) {
      this.qPayMethod.addEventListener("change", () => {
        ...
        if (this.qInstallments) {
          if (!canSplit) {
            this.qInstallments.value = "1";
          } else if (!String(this.qInstallments.value || "").trim()) {
            this.qInstallments.value = "1";
          }
        }
    ...
    if (this.qInstallments) {
      // Permite limpar/digitar livremente; normaliza apenas ao sair do campo (blur).
      this.qInstallments.setAttribute("inputmode", "numeric");
      this.qInstallments.setAttribute("pattern", "[0-9]*");

      this.qInstallments.addEventListener("input", () => {
        const cleaned = String(this.qInstallments.value || "").replace(/\D+/g, "");
        if (this.qInstallments.value !== cleaned) this.qInstallments.value = cleaned;
        this._updateInstallmentHint();
      });

      this.qInstallments.addEventListener("focus", () => {
        if (this.qInstallments.value === "1") this.qInstallments.select();
      });

      this.qInstallments.addEventListener("blur", () => {
        const raw = String(this.qInstallments.value || "").trim();
        let n = parseInt(raw, 10);
        if (!raw) n = 1;
        if (!Number.isFinite(n) || n < 1) n = 1;
        n = Math.max(1, Math.min(24, n));
        this.qInstallments.value = String(n);
        this._updateInstallmentHint();
      });

      this.qInstallments.addEventListener("change", () => {
        // garante normalização em browsers que disparam change sem blur
        this.qInstallments.dispatchEvent(new Event("blur"));
      });
    }
      ...
      row.innerHTML = `
        <div class="catHead">
          <div>
            ...
            <div class="muted small">Dia ${String(r.day).padStart(2,'0')} • ${r.type === 'entrada' ? 'Entrada' : 'Saída'} • ${fmtSource(r.source)} • ${escapeHtml(r.category)}${r.subcategory ? ` &gt; ${escapeHtml(r.subcategory)}` : ""} • a partir de ${Dates.monthKeyToBR(r.startMonth)}${r.endMonth ? ` • até ${Dates.monthKeyToBR(r.endMonth)}` : ``}${r.active ? '' : ' • (inativo)'}</div>
          </div>
  ...
  _updateInvoiceHint() {
    ...
    const invLabel = Dates.monthKeyToBR(invKey);
    const closingLabel = Dates.isoToBR(closingDate);
    const dueLabel = Dates.isoToBR(dueDate);
    this.qInvoiceHint.textContent = `Vai para fatura ${invLabel} • Fecha ${closingLabel} • Vence ${dueLabel}`;
  }
  ...
  _updateInstallmentHint() {
    ...
    // Enquanto o usuário digita, não forçar valor no input (evita bug 21).
    if (!raw) {
  ...
  _renderFatura() {
    ...
    for (const tx of items) {
      ...
      row.innerHTML = `
        ...
        <div class="txMeta">
          <div><span class="muted">Data:</span> ${Dates.isoToBR(tx.date || "")}</div>
  ...
  async _deleteTx(tx) {
    try {
      await this._requireLogin();
      // Recorrentes: se você só "excluir" o lançamento, ele volta a ser gerado no próximo refresh do mês.
      // Aqui damos opções para excluir só esta ocorrência (e impedir que seja gerada novamente) ou excluir a regra.
      if (tx && tx.recurringId) {
        const mk = String(tx.occurrenceMonth || Dates.monthKey(tx.date) || "").slice(0, 7);
        const choice = prompt(
          `Este é um lançamento RECORRENTE. O que deseja fazer?\n\n` +
          `1 = Excluir apenas ESTE mês (não volta a aparecer)\n` +
          `2 = Excluir a RECORRÊNCIA (regra)\n` +
          `0 = Cancelar`,
          "1"
        );
        ...
        if (c === "2") {
          // Remove the rule (does not delete past generated transactions)
          const rules = await this.recSvc.load();
          const rule = rules.find(r => String(r.id) === String(tx.recurringId));
          if (!rule) {
            alert("Regra recorrente não encontrada. Vou excluir apenas este lançamento.");
          } else {
            const okRule = confirm(`Excluir a recorrência "${rule.description}"?\n\nIsso NÃO apaga lançamentos já gerados.`);
            if (!okRule) return;
            const next = rules.filter(r => String(r.id) !== String(rule.id));
            await this.recSvc.save(next);
            // Também remove este lançamento atual
        ...
        // Default: exclude only this occurrence (skip regeneration) + delete tx doc
        const okTx = confirm(`Excluir "${tx.description}" (${Money.toBRL(tx.amount)})?\n\nObs.: por ser recorrente, vou marcar para NÃO gerar novamente em ${mk}.`);
        if (!okTx) return;
        if (mk) await this.recSvc.addExclusion(tx.recurringId, mk);
        await this.txSvc.remove(tx.id);
      } else {
